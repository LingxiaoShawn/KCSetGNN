...
class ParallelLayer(nn.Module):
    def __init__(self, nhid, mlp_num_layers=2, pools=['add'], num_bipartites=1,  half_step=False):
        ...
        self.linear2 = nn.Sequential(nn.Linear(len(pools)*nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
        self.linear3 = nn.Sequential(nn.Linear(len(pools)*nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
        self.linear4 = nn.Sequential(nn.Linear(len(pools)*nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())

...
class BipartiteGNN(nn.Module):
    def __init__(self, nin, nout, nlayer, type, mlp_layers=2, dropout=0, bias=True, pooling='add', num_bipartites=1, half_step=False):
        super().__init__()
        assert type in ['Sequential', 'Parallel']
        self.type = type 
        self.dropout = dropout
        Layer = SequentialLayer if type == 'Sequential' else ParallelLayer
        self.layers = nn.ModuleList(Layer(nin, mlp_num_layers=mlp_layers, pooling=pooling, num_bipartites=num_bipartites, half_step=half_step)  for _ in range(nlayer))
...
class ParallelLayer(nn.Module):
    def __init__(self, nhid, mlp_num_layers=2, pooling='add', num_bipartites=1,  half_step=False):
        super().__init__()
        self.pooling = pooling 
        self.half_step = half_step 
        self.nn1 = MLP(nhid, nhid, mlp_num_layers, with_final_activation=True) 
        self.nn2 = MLP(nhid, nhid, mlp_num_layers, with_final_activation=True) 
        self.linear1 = nn.Sequential(nn.Linear(nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
        self.linear2 = nn.Sequential(nn.Linear(nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
        self.linear3 = nn.Sequential(nn.Linear(nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
        self.linear4 = nn.Sequential(nn.Linear(nhid, nhid//2), nn.BatchNorm1d(nhid//2), nn.ReLU())
...
class SequentialLayer(nn.Module):
    def __init__(self, nhid, mlp_num_layers=2, pooling='add', num_bipartites=1, half_step=False):
        ...
        self.pooling = pooling
        self.combine1 = nn.ModuleList(SelfCombiner(nhid, mlp_num_layers) for _ in range(num_bipartites))
        self.combine2 = nn.ModuleList(SelfCombiner(nhid, mlp_num_layers) for _ in range(num_bipartites))    
        self.half_step = half_step
    def reset_parameters(self):
...
class SelfCombiner(nn.Module):
    def __init__(self, nhid, mlp_num_layers=2, type='2-way'):
        super().__init__()
        assert type in ['2-way', '3-way', '4-way']
        self.nn1 = MLP(nhid, nhid, mlp_num_layers, with_final_activation=True)
        if type == '4-way':
            self.nn2 = MLP(nhid, nhid, mlp_num_layers, with_final_activation=True)
            self.nn3 = MLP(nhid, nhid, mlp_num_layers, with_final_activation=True)
